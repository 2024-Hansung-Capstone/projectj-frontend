...
export default function Cooking() {
  ...
  const { loadingIngredients, errorIngredients, dataIngredients ,refetch} = useQuery(FETCH_MY_INGREDIENTS);
  ...
  const { Search } = Input;
  const [createIngredient, { data:dataCreated, loading:loadingCreated, error :errorCreated}] = useMutation(CREATE_INGREDIENT);
  useEffect(() => { // 토큰
  ...
  const handleAddIngredient = async (e) => {
    ...
    if (ingredientName && volume && volume_unit) {
      try {
        ...
        console.log(JSON.stringify(errorIngredients, null, 2))
      }
      setIngredient('');
      setQuantity('');
      setUnit('');
